...
def start(info):
    ...
    unoBot.safe_send_message(
        info.chat.id,
        'Этот бот предназначен для проведения игры уно в телеграмме. Для запуска игры добавьте бота в свою группу, и напишите /start_game.'
    )


def admin(info):
    unoBot.safe_send_message(info.chat.id,
                             'Создатели:\n@shout_0_0\n@rbedin25\n@n3tw4lk3r')


def stiker(info):
    '''Небольшая пасхалка.'''
    unoBot.safe_send_stiker(
        info.chat.id,
        'CAACAgIAAxkBAAEBpnZlPXSscqnvN_rM-uZusGxvanFG2wACuCQAArgGAUiH8Vp5cuhbHDAE'
    )


def win(info):
    '''он был умнее остальных'''
    logic.games_byId[info.chat.id].players[logic.games_byId[
        info.chat.id].current_position].cards = []


def join(info):
    '''Присоединение игрока после начала игры.'''
    if info.chat.id in logic.games_byId and info.from_user.username not in logic.games_byId[
            info.chat.id].player_hasActed:
        unoBot.safe_send_message(info.chat.id,
                                 f'Игрок {info.from_user.username} добавлен')
        logic.games_byId[info.chat.id].add_player(info.from_user.username,
                                                  info.from_user.id)
...
def end_game(info):
    ...
    if info.chat.id in logic.games_byId and logic.games_byId[
            info.chat.id].isRunning:
        if info.from_user.username in logic.games_byId[
                info.chat.id].player_hasActed:
...
def start_game(info):
    ...
    if logic.isGameRunning(info.chat.id):
        unoBot.safe_send_message(info.chat.id, 'Игра уже идёт.')
        return
    ...
    unoBot.safe_send_message(
        info.chat.id,
        'Нажмите присоединиться, чтобы войти в игру. Нажмите начать игру, для запуска игры.',
...
def play(info):
    ...
    if info.chat.id not in logic.games_byId or logic.games_byId[
            info.chat.id].isRunning or len(
                logic.games_byId[info.chat.id].players) == 0:
    ...
    sharedVars.threadsByChatId[info.chat.id] = Thread(
        target=logic.games_byId[info.chat.id].game)
...
def put_card(info):
    player = info.from_user.username
    for ind in range(
            len(logic.games_byId[info.chat.id].players[logic.games_byId[
                info.chat.id].current_position].cards)):
        if info.text == logic.games_byId[info.chat.id].players[
                logic.games_byId[
                    info.chat.id].current_position].cards[ind].name:
            logic.games_byId[info.chat.id].player_hasActed[player] = True
